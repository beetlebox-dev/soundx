...
def render_home_template(error=''):
    start_new_ferry_data_thread()
    return render_template('ferryhome.html', error=error)
...
def render_results_template(origin, destination):
    ...
    routes_info = get_info_all_routes(
        ...
        location_info['depart terminal'], ferry_data['schedules'], timestamp_now
    )
    ...
    admin_alert_thread('Web App - Log', f'SoundX\nRequest successful. Rendered ferryresults.html.\n'
                                        f'ORIGIN: {origin}\nDESTINATION: {destination}')
    return render_template(
        'ferryresults.html', data=routes_info, ferry_alerts=ferry_data['alerts'],
        origin=origin, destination=destination, time=format_timestamp(timestamp_now)
    )
...
<table>
    <tr>
        <td class="table-col-left">DEPART ORIGIN:</td>
        <td class="table-col-right">{format_timestamp(data["drive start timestamp"])} <span class="smaller">(in {data["leave in x minutes"]} minutes)</span></td>
    </tr>
    <tr>
        <td class="table-col-left">DRIVE TO FERRY:</td>
...
def get_info_all_routes(origin, destination, depart_terminal, ferry_schedule, timestamp_now):
    ...
    if route_2_drive_secs < 86400:  # Arriving less than 24 hours from now.
        ...
        route_2_html = f'''
<h3>VIA THE TACOMA NARROWS BRIDGE:<br><span class="smaller">(no ferry)</span></h3>
<table>
    <tr>
        <td class="table-col-left">DEPART ORIGIN:</td>
        <td class="table-col-right">{format_timestamp(timestamp_now)} <span class="smaller">(now)</span></td>
    </tr>
    <tr>
        <td class="table-col-left">DEPART ORIGIN:</td>
        <td class="table-col-right">Now</td>
